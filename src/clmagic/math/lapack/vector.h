		...
		if (_First != nullptr && _Last > _First) {
			...
			if (_Mybkoff % sizeof(_SclTy) == 0) {
				...
				if (_Pass_alignment_point) {// leading_size != 0 || block_size != 0
					_Dest.leading_size = (_Mybkoff == 0) ? 0 : block_traits<_BlkTy>::size() - _Mybkoff;
...
struct _Vector_operation<_SclTy, _BlkTy, _Vector_accelerate_level::Auto> {
	...
	static _OutIt func_1st(_InIt _First, _InIt _Last, _OutIt _Dest, _Fn _Func, 
		...
		if (_Vsize.can_fastest()) {
			...
			auto       _Dest_bk  = reinterpret_cast<_BlkTy*>( (_SclTy*)(_Dest) + _Vsize.leading_size );
	...
	static _OutIt func_1st(_InIt _First, _InIt _Last, _OutIt _Dest, _Fn _Func,
		...
		return norm_operation::func_1st((const _SclTy*)(_First), (const _SclTy*)(_Last), (_SclTy*)(_Dest), _Func, 
			_Inc1, _Inc2);
	}
	...
	static _OutIt func_2nd(_InIt _First1, _InIt _Last1, _InIt _First2, _OutIt _Dest, _Fn _Func,
		...
		if (_Vsize1 == _Vsize2) {
			if (_Vsize1.can_fastest()) {
				...
				auto       _Dest_bk   = reinterpret_cast<_BlkTy*>(      (_SclTy*)(_Dest)         + _Vsize1.leading_size);
	...
	static _SclTy& afunc_2nd(_InIt _First1, _InIt _Last1, _InIt _First2, _SclTy& _Result, _Afn _Afunc, _Fn _Func,
		...
		return norm_operation::afunc_2nd(static_cast<const _SclTy*>(_First1), static_cast<const _SclTy*>(_Last1),
			static_cast<const _SclTy*>(_First2), _Result, _Afunc, _Func, _Inc1, _Inc2);
	}
...
struct subvector : public const_subvector<_SclTy, _BlkTy> {// default const _SclTy
	...
	subvector& operator*=(const scalar_type& _Right) {
		...
		if (_Mybase::is_continue()) {
			...
			const auto _Func  = [&_Rhs](auto&& _Lhs) { return _Lhs * _Rhs; };
			if ( _Vsize.can_fastest() ) {
				fastest_vector_operation<_SclTy, _BlkTy>::func_1st(
					reinterpret_cast<const _BlkTy*>(_First), reinterpret_cast<const _BlkTy*>(_Last),
					reinterpret_cast<_BlkTy*>(_Dest),
					_Func);
				return *this;
			} else if( !_Vsize.empty() ) {
				auto       _First_bk = reinterpret_cast<const _BlkTy*>( (const _SclTy*)(_First) + _Vsize.leading_size );
				const auto _Last_bk  = _First_bk + _Vsize.block_size;
				auto       _Dest_bk  = reinterpret_cast<_BlkTy*>( (_SclTy*)(_First) + _Vsize.leading_size );
				fast_vector_operation<_SclTy, _BlkTy>::func_1st(_First_bk, _Last_bk, _Dest_bk, _Func,
					_Vsize.leading_size, _Vsize.tail_size);
				return *this;
			}
		}

		norm_vector_operation<_SclTy, _BlkTy>::func_1st(_First, _Last, _Dest, [&_Right](auto&& _Lhs) { return _Lhs * _Right; }, 
...
/*- - - - - - - - - - - - - - - - - - - vector<> - - - - - - - - - - - - - - - - - - - - -*/
#define make_clmagic_aligned_(NAME, ...) \
__VA_ARGS__ NAME;						 \
(NAME)._Correct_tail_elements()

// <temp-macro>
#define _VECTOR_FUNC_1ST_0(LEFT, RESULT, FUNC) \
_Generate_block_iter_1st(LEFT, RESULT);        \
::clmagic::fastest_vector_operation<_SclTy, _BlkTy>::func_1st(_First, _Last, _Dest, (##FUNC##))

#define _VECTOR_FUNC_1ST_1(LEFT, FUNC)   \
vector<_SclTy, _Size, _BlkTy> _Result;   \
_Result._Correct_tail_elements();        \
_VECTOR_FUNC_1ST_0(LEFT, _Result, FUNC); \
return _Result

#define _VECTOR_FUNC_2ND_0(LEFT, RIGHT, RESULT, FUNC) \
_Generate_block_iter_2nd(LEFT, RIGHT, RESULT);        \
::clmagic::fastest_vector_operation<_SclTy, _BlkTy>::func_2nd(_First1, _Last1, _First2, _Dest, (##FUNC##))

#define _VECTOR_FUNC_2ND_1(LEFT, RIGHT, FUNC)   \
vector<_SclTy, _Size, _BlkTy> _Result;          \
_Result._Correct_tail_elements();               \
_VECTOR_FUNC_2ND_0(LEFT, RIGHT, _Result, FUNC); \
return _Result
// </temp-macro>

template<typename _SclTy, size_t _Size, typename _BlkTy>
class __declspec(align(std::alignment_of_v<_BlkTy>)) _Data_for_vector {
...
public:
	...
	static constexpr size_t block_size() {
		return (_Real_size / block_traits<_BlkTy>::size());
	}
...
class vector : public _Data_for_vector<_SclTy, _Size, _BlkTy> {
	using _Mybase = _Data_for_vector<_SclTy, _Size, _BlkTy>;
...
public:
	...
	vector() = default;
	explicit 
	vector(const _SclTy& _Val) { _Mybase::assign(_Val); }
	vector(std::initializer_list<_SclTy> _Ilist) { _Mybase::assign(_Ilist.begin(), _Ilist.end()); }
	template<typename _Iter> 
	vector(_Iter _First, _Iter _Last) { _Mybase::assign(_First, _Last); }
	...
	vector operator-() const {
		_VECTOR_FUNC_1ST_1(*this, std::negate<_BlkTy>());
	}
	vector  operator+ (const vector& _Right) const {
		_VECTOR_FUNC_2ND_1(*this, _Right, std::plus<_BlkTy>());
	}
	vector  operator- (const vector& _Right) const {
		_VECTOR_FUNC_2ND_1(*this, _Right, std::minus<_BlkTy>());
	}
	vector  operator* (const vector& _Right) const {
		_VECTOR_FUNC_2ND_1(*this, _Right, std::multiplies<_BlkTy>());
	}
	vector  operator/ (const vector& _Right) const {
		_VECTOR_FUNC_2ND_1(*this, _Right, std::divides<_BlkTy>());
	}
	vector  operator% (const vector& _Right) const {
		_VECTOR_FUNC_2ND_1(*this, _Right, std::modulus<_BlkTy>());
	}
	vector& operator+=(const vector& _Right) {
		_VECTOR_FUNC_2ND_0(*this, _Right, *this, std::plus<_BlkTy>());
		return (*this);
	}
	vector& operator-=(const vector& _Right) {
		_VECTOR_FUNC_2ND_0(*this, _Right, *this, std::minus<_BlkTy>());
		return (*this);
	}
	vector& operator*=(const vector& _Right) {
		_VECTOR_FUNC_2ND_0(*this, _Right, *this, std::multiplies<_BlkTy>());
		return (*this);
	}
	vector& operator/=(const vector& _Right) {
		_VECTOR_FUNC_2ND_0(*this, _Right, *this, std::divides<_BlkTy>());
		return (*this);
	}
	vector& operator%=(const vector& _Right) {
		_VECTOR_FUNC_2ND_0(*this, _Right, *this, std::modulus<_BlkTy>());
		return (*this);
	}
	
	vector  operator+ (const _SclTy& _Scalar) const {
		const auto _Rhs = block_traits::set1(_Scalar);
		_VECTOR_FUNC_1ST_1(*this, [&_Rhs](auto&& _Lhs) { return _Lhs + _Rhs; });
	}
	vector  operator- (const _SclTy& _Scalar) const {
		const auto _Rhs = block_traits::set1(_Scalar);
		_VECTOR_FUNC_1ST_1(*this, [&_Rhs](auto&& _Lhs) { return _Lhs - _Rhs; });
	...
	vector& operator+=(const _SclTy& _Scalar) {
		const auto _Rhs = block_traits::set1(_Scalar);
		_VECTOR_FUNC_1ST_0(*this, *this, [&_Rhs](auto&& _Lhs) { return _Lhs + _Rhs; });
		return *this;
	}
	vector& operator-=(const _SclTy& _Scalar) {
		const auto _Rhs = block_traits::set1(_Scalar);
	...
	friend vector operator+(const _SclTy& _Scalar, const vector& _Right) {
		const auto _Lhs = block_traits::set1(_Scalar);
		_VECTOR_FUNC_1ST_1(_Right, [&_Lhs](auto&& _Rhs) { return _Lhs + _Rhs; });
	}
	friend vector operator-(const _SclTy& _Scalar, const vector& _Right) {
		const auto _Lhs = block_traits::set1(_Scalar);
		_VECTOR_FUNC_1ST_1(_Right, [&_Lhs](auto&& _Rhs) { return _Lhs - _Rhs; });
	...
	_SclTy sum() const {// X[0] + X[1] + X[2] + ... + X[N]
		_Generate_block_iter_nodest_1st( *this );
		_SclTy _Result = static_cast<_SclTy>(0);
		operation::afunc(_First, _Last, _Result, [](auto& _Res, auto&& _X) { _Res += _X; });
		return _Result;
	}

	_SclTy product() const {// X[0] * X[1] * X[2] * ... * X[N]
		_Generate_block_iter_nodest_1st(*this);
		_SclTy _Result = static_cast<_SclTy>(0);
		operation::afunc(_First, _Last, _Result, [](auto& _Res, auto&& _X) { _Res *= _X; });
		return _Result;
	...
	_SclTy norm(_SclTy L) const {
		_Generate_block_iter_nodest_1st( *this );
		_SclTy _Result = static_cast<_SclTy>(0);
		operation::norm(_First, _Last, L, _Result);
		return _Result;
	}
	
	_SclTy dot(const vector& _Right) const {// X[0]*Y[0] + X[1]*Y[1] + X[2]*Y[2] + ... * X[N]*Y[N]
		_Generate_iter_nodest_2nd(*this, _Right);
		_SclTy _Result = static_cast<_SclTy>(0);
		operation::afunc_2nd(_First1, _Last1, _First2, _Result, 
			[](auto& _Res, auto&& _X) { _Res += _X; }, std::multiplies<>());
		return _Result;
	}
	...
	vector& normalize() {
		const auto L2sq = (*this).dot(*this);
		if (!approach_equal(L2sq, static_cast<_SclTy>(1), std::numeric_limits<_SclTy>::epsilon())) {
			*this /= sqrt(L2sq);
		}
		return (*this);
	}
...
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
_SclTy sum(const vectorN& v) {
	return v.sum();
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
_SclTy product(const vectorN& v) {
	return v.product();
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
_SclTy dot(const vectorN& v1, const vectorN& v2) {
	return v1.dot(v2);
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
_SclTy length(const vectorN& v) {
	return v.length();
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
_SclTy norm(const vectorN& v, const _SclTy& L) {
	return v.norm(L);
}

template<typename _SclTy, size_t _Size, typename _BlkTy> inline
vectorN normalize(const vectorN& v) {
	const auto L2sq = dot(v, v);
	return !approach_equal(L2sq, static_cast<_SclTy>(1), std::numeric_limits<_SclTy>::epsilon()) ?
		v / sqrt(L2sq) :
		v;
}
...
vectorN mod(const vectorN& _Left, const vectorN& _Right) {
	_VECTOR_FUNC_2ND_1(_Left, _Right, std::modulus<_BlkTy>());
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
vectorN pow(const vectorN& _Left, const vectorN& _Right) {
	_VECTOR_FUNC_2ND_1(_Left, _Right, [](auto&& A, auto&& B) { return pow(A, B); });
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
...
vectorN abs(const vectorN& _Left) {
	_VECTOR_FUNC_1ST_1(_Left, [](auto&& A) { return abs(A); });
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
vectorN floor(const vectorN& _Left) {
	_VECTOR_FUNC_1ST_1(_Left, [](auto&& A) { return floor(A); });
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
vectorN ceil(const vectorN& _Left) {
	_VECTOR_FUNC_1ST_1(_Left, [](auto&& A) { return ceil(A); });
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
vectorN trunc(const vectorN& _Left) {
	_VECTOR_FUNC_1ST_1(_Left, [](auto&& A) { return trunc(A); });
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
vectorN round(const vectorN& _Left) {
	_VECTOR_FUNC_1ST_1(_Left, [](auto&& A) { return round(A); });
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
vectorN sqrt(const vectorN& _Left) {
	_VECTOR_FUNC_1ST_1(_Left, [](auto&& A) { return sqrt(A); });
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
vectorN cbrt(const vectorN& _Left) {
	_VECTOR_FUNC_1ST_1(_Left, [](auto&& A) { return cbrt(A); });
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
vectorN invsqrt(const vectorN& _Left) {
	_VECTOR_FUNC_1ST_1(_Left, [](auto&& A) { return invsqrt(A); });
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
vectorN invcbrt(const vectorN& _Left) {
	_VECTOR_FUNC_1ST_1(_Left, [](auto&& A) { return invcbrt(A); });
}

template<typename _SclTy, size_t _Size, typename _BlkTy> inline
vectorN sin(const vectorN& _Left) {
	_VECTOR_FUNC_1ST_1(_Left, [](auto&& A) { return sin(A); });
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
vectorN cos(const vectorN& _Left) {
	_VECTOR_FUNC_1ST_1(_Left, [](auto&& A) { return cos(A); });
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
vectorN tan(const vectorN& _Left) {
	_VECTOR_FUNC_1ST_1(_Left, [](auto&& A) { return tan(A); });
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
vectorN asin(const vectorN& _Left) {
	_VECTOR_FUNC_1ST_1(_Left, [](auto&& A) { return asin(A); });
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
vectorN acos(const vectorN& _Left) {
	_VECTOR_FUNC_1ST_1(_Left, [](auto&& A) { return acos(A); });
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
vectorN atan(const vectorN& _Left) {
	_VECTOR_FUNC_1ST_1(_Left, [](auto&& A) { return atan(A); });
}
template<typename _SclTy, size_t _Size, typename _BlkTy> inline
vectorN atan2(const vectorN& Vy, const vectorN& Vx) {
	_VECTOR_FUNC_2ND_1(Vy, Vx,
		[](auto&& y, auto&& x) { return atan2(x, x); });
}
...
#undef _VECTOR_FUNC_1ST_0
#undef _VECTOR_FUNC_1ST_1
#undef _VECTOR_FUNC_2ND_0
#undef _VECTOR_FUNC_2ND_1

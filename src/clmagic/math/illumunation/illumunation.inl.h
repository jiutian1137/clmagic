...
namespace clmagic 
{
	...
	template<typename T> inline
		...
		{
		return (normalize(Vec3_<T>(T(0), T(sin(_Time)), T(cos(_Time)))));
		}
	...
	template<typename T> inline
		Vec3_<T> BlinnPhong_specalbedo(_in(Vec3_<T>) _Obj_Rf, _in(Vec3_<T>) _Obj_N, _in(T) _Obj_Shininess,
			_in(Vec3_<T>) _Tolight, _in(Vec3_<T>) _Toeye)
		{	// Schlick_Fresnel(Rf, N, L) * { ((m+8)/8) * Lambert(H, N)^m }
		Vec3_<T> _Halfvec = normalize(_Tolight + _Toeye);
	...
	template<typename T> inline
		Vec3_<T> BlinnPhong( _in(Vec3_<T>) _Lstrenth, _in(Vec3_<T>) _Objr0, _in(Vec3_<T>) _Objnormal, _in(T) _Objroughness, _in(Vec3_<T>) _Objdiffuse,
			_in(Vec3_<T>) _Tolight, _in(Vec3_<T>) _Toeye) 
		{	// _Lstrenth * (_Objdiffuse + BlinnPhong_specalbedo(_Objr0, _Objnormal, _Objroughness, _Tolight, _Toeye))
		Vec3_<T> _Specalbedo = BlinnPhong_specalbedo(_Objr0, _Objnormal, _Objroughness, _Tolight, _Toeye);

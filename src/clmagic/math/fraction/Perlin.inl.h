...
namespace clmagic
{
	...
	*/
	...
	template<typename _Ty> inline
		_Ty cnoise2(Vec2_<_Ty> _Pos)
		{
		Vec4_<_Ty> _Pi = floor(Vec4_<_Ty>(_Pos.x, _Pos.y, _Pos.x, _Pos.y)) + Vec4_<_Ty>(_Ty(0), _Ty(0), _Ty(1), _Ty(1));
		Vec4_<_Ty> _Pf = fract(Vec4_<_Ty>(_Pos.x, _Pos.y, _Pos.x, _Pos.y)) - Vec4_<_Ty>(_Ty(0), _Ty(0), _Ty(1), _Ty(1));
		_Pi = _Pi % _Ty(289); // To avoid truncation effects in permutation
		Vec4_<_Ty> ix = Vec4_<_Ty>(_Pi.x, _Pi.z, _Pi.x, _Pi.z);//_Pi.xzxz
		Vec4_<_Ty> iy = Vec4_<_Ty>(_Pi.y, _Pi.y, _Pi.w, _Pi.w);//_Pi.yyww
		Vec4_<_Ty> fx = Vec4_<_Ty>(_Pf.x, _Pf.z, _Pf.x, _Pf.z);//_Pf.xzxz
		Vec4_<_Ty> fy = Vec4_<_Ty>(_Pf.y, _Pf.y, _Pf.w, _Pf.w);//_Pf.yyww
		...
		Vec2_<_Ty> g00(gx.x, gy.x);
		Vec2_<_Ty> g10(gx.y, gy.y);
		Vec2_<_Ty> g01(gx.z, gy.z);
		Vec2_<_Ty> g11(gx.w, gy.w);

...
namespace clmagic 
{
	template<typename T> inline
		Vec3_<T> Ray_<T>::operator()(_in(T) t) const
		{ return (o + t * d); }
	...
	*/
	...
	template<typename T> inline
		bool _Clip_test_spherical(_in(Ray_<T>) _Ray, _in(Sphere_<T>) _Sphere,
			/*in*/T t0, /*in*/T t1, _in(Vec3_<T>) o, _in(Vec3_<T>) d,
			_out(T) t, _out(Vec3_<T>) impack, _out(T) phi, _out(T) theta)
		{
		sort(t0, t1);
		t      = t0;
		impack = o + t * d;
		phi    = atan(impack.y, impack.x); if (phi < T(0)) phi += T(c2Pi);
	...
	template<typename T> inline
		bool intersect(_in(Ray_<T>) _Ray, _in(Sphere_<T>) _Sphere, _out(Hit_<T>) _Result) {
			...
			const auto o = _Sphere.world.inv_transform(_Ray.o);
			const auto d = _Sphere.world.inv_transform(_Ray.d, _Tag_normal());
			T t0, t1;
			T t; Vec3_<T> impack; T phi; T theta;
			...
			if ( spherical_equation(o, d, _Sphere.r, t0, t1 ) ) {
				if ( _Clip_test_spherical(_Ray, _Sphere, t0, t1, o, d, /*out..*/t, impack, phi, theta) ) {
					...
					_Result._Myt      = t;
					_Result._Myorigin = _Sphere.world.transform(impack);
					_Result._Mynormal = _Sphere.world.transform(impack / _Sphere.r, _Tag_normal());
	...
	template<typename T> inline
		bool intersect(_in(Ray_<T>) _Ray, _in(Triangle_<T>) _Triangle, _out(Hit_<T>) _Result) {
			...
			[b2]       s1，e1    [s2，d ]
			*/
			Vec3_<T> _e1 = _Triangle._Myp1 - _Triangle._Myp0;
			Vec3_<T> _e2 = _Triangle._Myp2 - _Triangle._Myp0;
			Vec3_<T> _s  = _Ray.o - _Triangle._Myp0;

			Vec3_<T> _s1 = cross(_Ray.d, _e2);
			T _Invdet = dot(_s1, _e1); if (equals(_Invdet, T(0), T(0.00002))) return ( false );
			_Invdet = T(1) / _Invdet;
	...
	template<typename T> inline
	Vec3_<T>& PerspectiveRay_<T>::forward_vector() {
		return ( _Mylook.d );
	}

	template<typename T> inline
	Vec3_<T>& PerspectiveRay_<T>::up_vector() {
		return ( _Myup );
	}

	template<typename T> inline
	Vec3_<T>& PerspectiveRay_<T>::right_vector() {
		return (_Myright);
	}

	template<typename T> inline
	Mat_<4, 4, T>& PerspectiveRay_<T>::view_matrix() {
		return (_Myview);
	}

	template<typename T> inline
	Mat_<4, 4, T>& PerspectiveRay_<T>::projection_matrix() {
		return (_Myproj);
	}

	template<typename T> inline
	const Vec3_<T>& PerspectiveRay_<T>::forward_vector() const {
		return (_Mylook.d);
	}

	template<typename T> inline
	const Vec3_<T>& PerspectiveRay_<T>::up_vector() const {
		return (_Myup);
	}

	template<typename T> inline
	const Vec3_<T>& PerspectiveRay_<T>::right_vector() const {
		return (_Myright);
	}

	template<typename T> inline
	const Mat_<4, 4, T>& PerspectiveRay_<T>::view_matrix() const {
		return (_Myview);
	}

	template<typename T> inline
	const Mat_<4, 4, T>& PerspectiveRay_<T>::projection_matrix() const {
		return (_Myproj);
	}
}// namespace clmagic
